...
pub fn get_esp_volume(boot_services: &BootServices) -> Result<Handle, Status> {
    let handles = boot_services
        .locate_handle_buffer(uefi::table::boot::SearchType::ByProtocol(&SimpleFileSystem::GUID))
        .map_err(|e| e.status())?;
...
pub fn load_config(boot_services: &BootServices, esp_handle: &Handle) -> Result<InferenceConfig, Status> {
    
    let mut sfs = boot_services.open_protocol_exclusive::<SimpleFileSystem>(*esp_handle)
        .map_err(|e| e.status())?;
    let mut root = sfs.open_volume().map_err(|e| e.status())?;
    
    let config_path = cstr16!("\\inference_config.json");
    let file = root.open(config_path, FileMode::Read, FileAttribute::empty())
        .map_err(|e| e.status())?;
    ...
    serial_println!("📄 Loaded inference config: runtime={:?}, context={}", config.runtime, config.context_length);
    Ok(config)
}

pub fn load_model(boot_services: &BootServices, esp_handle: &Handle, model_path: &str) -> Result<Vec<u8>, Status> {
    ...
    let file = root.open(path_cstr, FileMode::Read, FileAttribute::empty())
        .map_err(|e| e.status())?;
    ...
    serial_println!("🧠 Loaded AI model: {} bytes from {}", file_size, model_path);
    Ok(buffer)
}
