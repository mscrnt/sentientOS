...
pub fn launch_kernel(
    ...
    {
        let mut loaded_image = match boot_services
        ...
        unsafe {
            loaded_image.set_load_options(
                args_vec.as_ptr() as *const u8,
                (args_vec.len() * 2) as u32
            );
        }
    }
    
    info!("Starting kernel image...");
    serial_println!("🎯 Starting kernel with bootinfo at 0x{:016x}", boot_info_addr);
...
fn store_boot_info(boot_services: &BootServices, boot_info: BootInfo) -> Result<u64, Status> {
    let serialized = serde_json::to_vec(&boot_info)
    ...
    serial_println!("📦 BootInfo stored at: 0x{:016x} ({} bytes)", address, serialized.len());
    Ok(address)
}