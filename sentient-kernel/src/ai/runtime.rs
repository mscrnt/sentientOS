...
use crate::boot_info::{InferenceConfig, ModelInfo};
...
impl ModelRuntime {
    ...
    pub fn infer(
        ...
        match request {
            ...
            InferenceRequest::PowerManagement { metrics } => self.manage_power(metrics, config),

            InferenceRequest::PanicAnalysis {
                location,
                line,
                message,
    ...
    fn analyze_system(
        ...
        match event {
            "kernel_boot" => Ok(InferenceResponse::DiagnosticInfo(String::from(
                "System initialized successfully. AI monitoring active.",
            ))),
            _ => {
                // Analyze metrics and provide recommendations
                if metrics.free_memory < 100 * 1024 * 1024 {
                    // Less than 100MB free
                    Ok(InferenceResponse::SystemCommand(String::from(
                        "optimize_memory",
                    )))
                } else {
                    Ok(InferenceResponse::DiagnosticInfo(alloc::format!(
                        "System healthy. Uptime: {}ms, Free memory: {}MB",
                        metrics.uptime_ms,
